...
const BASE_CONCEPTS: Concept[] = [
  ...
  },
  {
    ...
    details: {
      fr: "Livrer plus tôt accélère la matérialisation de la valeur (valeur temps de l'argent) : c'est pourquoi la livraison incrémentale est privilégiée dès que possible, même en contexte prédictif. Le plan de management des bénéfices trace comment et QUAND chaque bénéfice sera mesuré — souvent bien après la fin du projet, ce qui implique un propriétaire des bénéfices désigné qui survit à la clôture du projet. Un business case n'est jamais figé : un changement de contexte (marché, réglementation, stratégie) peut l'invalider en cours de route, et le chef de projet a le devoir d'alerter la gouvernance plutôt que de poursuivre par inertie. Point d'examen à bien distinguer : la réussite d'un projet se mesure sur DEUX dimensions indépendantes. La réussite des RÉSULTATS (outcome success) mesure l'efficacité de la création de valeur entrepreneuriale — ventes, ROI, position sur le marché, conformité — et peut se constater pendant, juste après, ou longtemps après le projet. La réussite des PROCESSUS de management (process success) mesure le respect des contraintes : budget, périmètre, échéancier, qualité. Ces deux dimensions sont INDÉPENDANTES : un projet peut réussir sur l'une et échouer sur l'autre.",
      en: 'Delivering earlier accelerates value materialization (time value of money): this is why incremental delivery is favored whenever possible, even in a predictive context. The benefits management plan tracks how and WHEN each benefit will be measured — often well after project closure, which implies a designated benefits owner who outlives the project. A business case is never fixed: a change in context (market, regulation, strategy) can invalidate it mid-course, and the project manager has a duty to alert governance rather than continue out of inertia. Exam point worth distinguishing clearly: project success is measured on TWO independent dimensions. OUTCOME success measures how effectively business value was created — sales, ROI, market position, compliance — and can be observed during, right after, or long after the project. Management PROCESS success measures adherence to constraints: budget, scope, schedule, quality. These two dimensions are INDEPENDENT: a project can succeed on one and fail on the other.',
    },
    examples: {
      fr: "• Publier un sous-ensemble de fonctionnalités plus tôt pour commencer à générer des revenus plutôt que d'attendre un lancement « big bang ».\n• Annuler une fonctionnalité en cours de projet parce qu'un mouvement concurrent a supprimé sa valeur attendue.\n• Mesurer les bénéfices six mois après la mise en service (taux d'adoption, économies réalisées) plutôt que de déclarer la victoire dès le déploiement.\n• L'Opéra de Sydney : prévu à 7 M AUD sur 4 ans, livré à 102 M AUD en 14 ans → échec cuisant du PROCESSUS, mais réussite éclatante du RÉSULTAT (site classé UNESCO, icône mondiale qui génère toujours de la valeur).\n• Le viaduc de Montréal : mal aligné avec la conception du pont, démoli un an après sa livraison pour 11 M CAD → même si le projet avait respecté délais et budget, l'échec du RÉSULTAT (aucune valeur durable créée) rend le processus sans objet.",
      en: 'Releasing a subset of features early to start generating revenue instead of waiting for a “big bang” launch.\nCanceling a feature mid-project because a competitor’s move eliminated its expected value.\nMeasuring benefits six months after go-live (adoption rate, realized savings) rather than declaring victory at deployment.\nThe Sydney Opera House: budgeted at AUD 7M over 4 years, delivered at AUD 102M over 14 years → a stark PROCESS failure, yet a resounding OUTCOME success (a UNESCO World Heritage site, a global icon still creating value today).\nThe Montreal viaduct: poorly aligned with the bridge’s design, demolished one year after delivery for CAD 11M → even had the project hit its schedule and budget, the OUTCOME failure (no lasting value created) renders the process moot.',
    },
  },
  {
    id: 'pr-systems',
  ...
  },
  {
    ...
    details: {
      fr: "Le PMBOK® distingue quatre sources principales de complexité : le comportement humain (motivations et interactions imprévisibles), le comportement du système (interdépendances non linéaires), l'incertitude (probabilités inconnues) et l'ambiguïté (relation de cause à effet floue, ou existence de plusieurs interprétations possibles). Chaque source appelle une réponse différente : un spike technique ou un prototype réduit l'incertitude ; réunir des points de vue diversifiés aide à interpréter une situation ambiguë ; découpler l'architecture limite la propagation en cascade d'une défaillance locale. La complexité ne se « résout » pas une fois pour toutes : elle se surveille en continu et se réduit progressivement au fil du projet. Le modèle de complexité de Stacey donne un outil de diagnostic complémentaire, sur deux axes : la certitude des EXIGENCES (proche de l'accord ↔ loin de l'accord) et la certitude de l'APTITUDE TECHNIQUE à les réaliser (proche de la certitude ↔ loin de la certitude). Quatre zones en résultent : SIMPLE (exigences et technique claires) → les approches linéaires/prédictives fonctionnent bien ; COMPLIQUÉ (une des deux dimensions est incertaine) → l'expertise et l'analyse suffisent ; COMPLEXE (les deux sont incertaines) → les approches adaptatives fonctionnent bien, on avance par expérimentation ; CHAOTIQUE (loin de l'accord ET loin de la certitude) → fondamentalement risqué, il faut stabiliser la situation avant de pouvoir planifier quoi que ce soit.",
      en: "The PMBOK® identifies four main sources of complexity: human behavior (unpredictable motivations and interactions), system behavior (non-linear interdependencies), uncertainty (unknown probabilities) and ambiguity (unclear cause-and-effect relationships, or multiple possible interpretations). Each source calls for a different response: a technical spike or prototype reduces uncertainty; bringing together diverse perspectives helps interpret an ambiguous situation; decoupling the architecture limits the cascading spread of a local failure. Complexity is never “solved” once and for all: it is continuously monitored and progressively reduced throughout the project. The Stacey complexity model offers a complementary diagnostic tool, on two axes: certainty of REQUIREMENTS (close to agreement ↔ far from agreement) and certainty of the TECHNICAL APTITUDE to deliver them (close to certainty ↔ far from certainty). Four zones result: SIMPLE (both clear) → linear/predictive approaches work well; COMPLICATED (one dimension uncertain) → expertise and analysis suffice; COMPLEX (both uncertain) → adaptive approaches work well, progress comes through experimentation; CHAOTIC (far from agreement AND far from certainty) → fundamentally risky, the situation must be stabilized before any planning is possible.",
    },
    examples: {
      fr: "• Réaliser un spike technique de deux jours avant de s'engager sur une technologie inconnue, pour réduire l'incertitude.\n• Découper un système monolithique en services faiblement couplés afin qu'une panne d'un module ne se propage pas en cascade.\n• Réunir des personnes de différents services pour interpréter une exigence réglementaire ambiguë.\n• Des exigences bien définies mais une technologie jamais utilisée par l'équipe → zone COMPLIQUÉE : faire appel à un expert externe ou former l'équipe, pas besoin d'itérer à l'aveugle.\n• Des exigences qui évoluent sans cesse ET une technologie encore instable → zone COMPLEXE : privilégier de courtes itérations avec retours fréquents plutôt qu'un plan détaillé figé d'avance.",
      en: "Running a two-day technical spike before committing to an unfamiliar technology, to reduce uncertainty.\nSplitting a monolithic system into loosely coupled services so a failure in one module does not cascade.\nBringing together people from different departments to interpret an ambiguous regulatory requirement.\nWell-defined requirements but a technology the team has never used → COMPLICATED zone: bring in an outside expert or train the team, no need to iterate blindly.\nRequirements that keep shifting AND still-unstable technology → COMPLEX zone: favor short iterations with frequent feedback over a detailed plan fixed in advance.",
    },
  },
  {
    id: 'pr-risk',
  ...
  // ──────────────────────── PERFORMANCE DOMAINS ────────────────────────
  {
    ...
    details: {
      fr: "La matrice d'évaluation de l'engagement se lit en deux colonnes : l'état ACTUEL (C) de chaque partie prenante et son état SOUHAITÉ (D). L'écart entre les deux colonnes révèle où concentrer les efforts d'engagement. Trois styles de communication complètent la boîte à outils : la communication push (envoyer l'information, ex. rapport, courriel), pull (mettre l'information à disposition, ex. intranet, portail) et interactive (échanger en direct, ex. réunion, atelier) — le bon style dépend de l'urgence, de la complexité et du nombre de destinataires. Le registre des parties prenantes n'est jamais figé : il doit être mis à jour dès qu'un événement change la carte des pouvoirs (réorganisation, nouveau régulateur, changement de sponsor). Au-delà de la grille pouvoir/intérêt, le cube des parties prenantes ajoute une troisième dimension (intérêt, pouvoir, influence) pour cartographier des situations trop complexes pour deux axes seulement. Le modèle de saillance se complète d'un repère directionnel utile à l'examen : l'influence s'exerce vers le HAUT (management senior), vers le BAS (membres de l'équipe), vers l'EXTÉRIEUR (fournisseurs, régulateurs, public, utilisateurs finaux) ou LATÉRALEMENT (pairs, autres chefs de projet) — reconnaître la direction aide à choisir la bonne stratégie. La matrice d'engagement distingue précisément CINQ niveaux, du plus faible au plus fort : non informé (ignore le projet — risque de surprise si non traité), résistant (conscient mais opposé, bloque activement), neutre (conscient, ni pour ni contre, implication passive), soutenant (comprend les bénéfices, coopère activement) et leader (défend le projet, influence positivement les autres). L'objectif du chef de projet change à chaque niveau : accroître la sensibilisation pour le non informé, traiter les préoccupations et bâtir la confiance pour le résistant et le neutre, encourager la participation pour le soutenant, maintenir l'engagement pour le leader.",
      en: 'The stakeholder engagement assessment matrix reads in two columns: each stakeholder’s CURRENT state (C) and their DESIRED state (D). The gap between the two columns reveals where to focus engagement efforts. Three communication styles complete the toolkit: push (sending information, e.g., a report or email), pull (making information available, e.g., an intranet or portal) and interactive (exchanging live, e.g., a meeting or workshop) — the right style depends on urgency, complexity and the number of recipients. The stakeholder register is never static: it must be updated whenever an event reshuffles the power map (reorganization, new regulator, change of sponsor). Beyond the power/interest grid, the stakeholder cube adds a third dimension (interest, power, influence) to map situations too complex for two axes alone. The salience model is completed by a directional lens the exam favors: influence flows UPWARD (senior management), DOWNWARD (team members), OUTWARD (vendors, regulators, the public, end users) or LATERALLY (peers, other project managers) — recognizing the direction helps pick the right strategy. The engagement matrix distinguishes exactly FIVE levels, weakest to strongest: unaware (does not know the project exists — risk of surprise if left untreated), resistant (aware but opposed, actively blocking), neutral (aware, neither for nor against, passive involvement), supportive (understands the benefits, cooperates actively), and leading (champions the project, positively influences others). The PM’s goal shifts at each level: raise awareness for the unaware, address concerns and build trust for the resistant and neutral, encourage participation for the supportive, sustain engagement for the leading.',
    },
    examples: {
      fr: "• Marquer l'engagement actuel d'une partie prenante comme « résistant » (C) avec un état souhaité « neutre » (D), puis planifier des actions ciblées pour combler l'écart.\n• Envoyer un rapport technique détaillé (pull) à un cadre peu intéressé tout en animant un atelier interactif avec un groupe d'utilisateurs impliqués.\n• Mettre à jour le registre des parties prenantes après avoir découvert qu'un organisme réglementaire non répertorié doit approuver le lancement.\n• Une partie prenante « leader » qui défend publiquement le projet auprès d'autres cadres → l'objectif du chef de projet est de MAINTENIR cet engagement, pas de la « gérer de près » comme on le ferait pour une partie prenante résistante.\n• Un régulateur externe qui bloque une autorisation exerce une influence vers l'EXTÉRIEUR ; un autre chef de projet qui doit partager la même ressource exerce une influence LATÉRALE — les deux exigent des stratégies différentes.",
      en: 'Marking a stakeholder’s current engagement as “resistant” (C) with a desired state of “neutral” (D), then planning targeted actions to close the gap.\nSending a detailed technical report (pull) to an uninterested executive while running an interactive workshop with an engaged user group.\nUpdating the stakeholder register after discovering an unlisted regulatory body must approve the launch.\nA “leading” stakeholder who publicly champions the project to other executives → the PM’s goal is to SUSTAIN that engagement, not “manage closely” as with a resistant stakeholder.\nAn external regulator blocking an approval exerts OUTWARD influence; another PM who must share the same resource exerts LATERAL influence — each calls for a different strategy.',
    },
  },
  {
    id: 'pd-team',
  ...
  // ─────────────────────────── EXAM TOPICS ───────────────────────────
  {
    id: 'tp-eco',
    category: 'topic',
    title: { fr: "Structure de l'examen (ECO 2026)", en: 'Exam structure (2026 ECO)' },
    summary: {
      fr: '180 questions (170 notées) réparties sur 3 domaines : Personnes 33 %, Processus 41 %, Environnement commercial 26 %.',
      en: '180 questions (170 scored) across 3 domains: People 33%, Process 41%, Business Environment 26%.',
    },
    body: {
      fr: "Depuis la mise à jour de juillet 2026, l'ECO (Examination Content Outline) définit 3 domaines et 26 tâches : I. Personnes — 33 % (8 tâches, ≈ 61 questions) ; II. Processus — 41 % (10 tâches, ≈ 76 questions) ; III. Environnement commercial — 26 % (8 tâches, ≈ 48 questions). L'examen comporte 180 questions en 240 minutes (170 notées + 10 questions de pré-test non notées, mélangées sans distinction), avec deux pauses de 10 minutes. Environ 40 % des questions portent sur des approches prédictives, 60 % sur des approches adaptatives/agiles ou hybrides — cette répartition traverse les 3 domaines, elle ne leur est pas propre.",
      en: 'Since the July 2026 update, the ECO (Examination Content Outline) defines 3 domains and 26 tasks: I. People — 33% (8 tasks, ≈ 61 questions); II. Process — 41% (10 tasks, ≈ 76 questions); III. Business Environment — 26% (8 tasks, ≈ 48 questions). The exam has 180 questions in 240 minutes (170 scored + 10 unscored pretest items, shuffled indistinguishably), with two 10-minute breaks. About 40% of questions involve predictive approaches, 60% adaptive/agile or hybrid — this split cuts across all 3 domains rather than belonging to one.',
    },
    details: {
      fr: "Les anciens chiffres (Personnes 42 %/Processus 50 %/Environnement d'affaires 8 %, 230 minutes) datent de l'ECO 2021 et ne sont PLUS valides depuis juillet 2026 — ne confondez pas les deux versions en révisant. Les 10 questions de pré-test sont indiscernables des questions notées : impossible de les repérer, donc chaque question doit être traitée avec le même sérieux. Deux types de questions sont NOUVEAUX en 2026 : les questions de cas/scénario (un contexte détaillé, parfois avec graphique, suivi d'une série de questions liées) et les questions basées sur un graphique/tableau à interpréter avant de répondre. Les autres formats restent : choix unique, réponses multiples, pointer-cliquer (hotspot), appariement (matching, avec glisser-déposer sur CBT), liste déroulante. Les deux pauses de 10 minutes ne tombent pas au même endroit qu'avant : la première suit la section des études de cas, la seconde arrive à mi-parcours des questions indépendantes — une fois la pause commencée, impossible de revenir aux questions de la section précédente. La philosophie de l'examen reste inchangée : pas « qu'a fait ce chef de projet dans le passé » mais « que DEVRAIT faire un chef de projet compétent maintenant », d'où la préférence systématique pour comprendre et analyser avant d'agir ou d'escalader.",
      en: 'The old figures (People 42%/Process 50%/Business Environment 8%, 230 minutes) come from the 2021 ECO and are NO LONGER valid since July 2026 — do not mix the two versions while revising. The 10 pretest items are indistinguishable from scored ones: there is no way to spot them, so every question must be treated with equal care. Two question types are NEW in 2026: case/scenario questions (a detailed context, sometimes with a graphic, followed by a series of linked questions) and graphic/table-based questions that must be interpreted before answering. Other formats remain: single choice, multiple response, point-and-click (hotspot), matching (drag-and-drop on CBT), dropdown list. The two 10-minute breaks no longer fall where they used to: the first follows the case-study section, the second arrives midway through the independent questions — once a break starts, you cannot return to the previous section’s questions. The exam’s philosophy is unchanged: not “what did this project manager do in the past” but “what SHOULD a competent project manager do now”, hence the systematic preference for understanding and analyzing before acting or escalating.',
    },
    examples: {
      fr: "• Une question décrivant un conflit dans une équipe Scrum reste classée dans le domaine « Personnes », même s'il s'agit d'un scénario agile.\n• Face à deux réponses plausibles, choisir « recueillir de l'information / comprendre la cause racine » plutôt que « escalader immédiatement » reflète le raisonnement privilégié par l'examen.\n• Une question de cas présente un projet avec un tableau de bord ; trois questions liées suivent, chacune s'appuyant sur les mêmes données — relire le scénario avant chaque sous-question évite les erreurs d'inattention.\n• Gérer son temps : ~80 secondes par question en moyenne (240 min / 180 questions) ; marquer et passer une question bloquante plutôt que de s'enliser.",
      en: 'A question describing a conflict in a Scrum team is still classified under the “People” domain, even though it is an agile scenario.\nFaced with two plausible answers, choosing “gather information / understand the root cause” over “escalate immediately” reflects the exam’s preferred reasoning.\nA case-study question presents a project with a dashboard; three linked questions follow, each relying on the same data — re-reading the scenario before each sub-question avoids careless mistakes.\nManaging time: ~80 seconds per question on average (240 min / 180 questions); flag and move on from a blocking question rather than getting stuck.',
    },
  },
  {
    id: 'tp-evm',
  ...
  },
  {
    ...
    details: {
      fr: "Aucune des cinq techniques n'est mauvaise en soi : chacune convient à un contexte précis, et le choisir correctement est ce que l'examen évalue. Forcer est justifié uniquement en situation d'urgence réelle (sécurité, conformité légale immédiate) — jamais comme réflexe de gain de temps. Éviter peut être temporairement approprié quand les émotions sont trop vives pour un dialogue constructif immédiat, à condition d'y revenir vite. Aplanir soulage la tension à court terme mais ne résout rien sur le fond : utile pour préserver une relation en attendant un moment plus propice à la collaboration. Le compromis est un pis-aller rapide sous contrainte de temps, pas un idéal à rechercher. Les niveaux de conflit de Leas donnent une échelle utile pour calibrer l'intervention : niveau 1, un problème à résoudre (factuel, se règle par la collaboration) ; niveau 2, un désaccord (les positions personnelles s'invitent, encore gérable en collaboration) ; niveau 3, une rivalité/contestation (chacun cherche à « gagner », le compromis ou la médiation devient nécessaire) ; niveau 4, lutte/fuite (les personnes veulent éliminer ou quitter l'autre partie, intervention formelle requise) ; niveau 5, une situation inextricable (quasi impossible à résoudre, nécessite une intervention externe ou une réorganisation). Plus le conflit escalade, plus tôt il faut intervenir : agir au niveau 1-2 avec la collaboration évite d'atteindre les niveaux 4-5.",
      en: "None of the five techniques is inherently bad: each fits a specific context, and choosing correctly is exactly what the exam tests. Forcing is justified only in a genuine emergency (safety, immediate legal compliance) — never as a time-saving reflex. Avoiding can be temporarily appropriate when emotions run too high for immediate constructive dialogue, provided you return to it quickly. Smoothing relieves short-term tension but resolves nothing substantively: useful for preserving a relationship while waiting for a better moment to collaborate. Compromise is a quick stopgap under time pressure, not an ideal to strive for. Leas's conflict levels give a useful scale for calibrating intervention: level 1, a problem to solve (factual, settled through collaboration); level 2, a disagreement (personal positions creep in, still manageable through collaboration); level 3, a contest/rivalry (each side seeks to “win”, compromise or mediation becomes necessary); level 4, fight/flight (people want to eliminate or leave the other party, formal intervention required); level 5, an intractable situation (nearly impossible to resolve, needs external intervention or reorganization). The more a conflict escalates, the earlier you must intervene: acting at level 1-2 with collaboration prevents reaching levels 4-5.",
    },
    examples: {
      fr: "• Collaboration : deux leads reconcoivent ensemble l'interface après avoir cartographié leurs préoccupations sous-jacentes respectives.\n• Compromis : partager un budget contesté à 50/50 comme solution rapide et temporaire sous contrainte de temps.\n• Forcer : un responsable sécurité impose l'arrêt immédiat d'un travail dangereux, justifié uniquement par l'urgence réelle de la situation.\n• Deux membres d'équipe ne sont plus d'accord sur une approche technique (niveau 2, désaccord) → une session de collaboration structurée suffit largement, pas besoin d'escalader.\n• Deux membres refusent désormais de travailler ensemble et l'un demande son transfert (niveau 4, lutte/fuite) → le chef de projet doit intervenir formellement, au-delà d'une simple médiation informelle.",
      en: "Collaborating: two leads jointly redesign the interface after mapping out both their underlying concerns.\nCompromising: splitting a disputed budget 50/50 as a quick, temporary fix under time pressure.\nForcing: a safety officer overrides preferences to immediately halt unsafe work, justified only by the genuine urgency of the situation.\nTwo team members disagree on a technical approach (level 2, disagreement) → a structured collaboration session is plenty, no need to escalate.\nTwo members now refuse to work together and one requests a transfer (level 4, fight/flight) → the PM must intervene formally, beyond simple informal mediation.",
    },
  },
  {
    id: 'tp-change',
  ...
  },
  {
    ...
    details: {
      fr: "Un moyen mnémotechnique utile : les cinq réponses aux menaces (éviter, transférer, atténuer, accepter, escalader) ont chacune un miroir presque symétrique côté opportunités (éviter → exploiter, transférer → partager, atténuer → améliorer, accepter → accepter, escalader → escalader) — mémoriser la structure côté menace permet de retrouver l'équivalent côté opportunité. Le registre des risques (working document détaillé, mis à jour en continu) doit être distingué du rapport des risques (synthèse destinée à la gouvernance, présentant l'exposition globale du projet). L'escalade signifie transmettre un risque à un niveau de décision qui dépasse l'autorité du chef de projet (programme, portefeuille, direction) — ce n'est ni une réponse de menace ni un aveu d'échec, c'est la réponse correcte quand le risque sort du périmètre d'action du projet. Point d'examen central : RISQUE et POINT À TRAITER (issue) ne sont PAS synonymes. Un risque est orienté vers l'AVENIR (un événement qui pourrait se produire), peut être positif OU négatif, se consigne dans le registre des risques, et sa réponse s'appelle une « réponse au risque ». Un point à traiter est orienté vers le PRÉSENT (une situation qui existe déjà), est TOUJOURS négatif, se consigne dans le journal des points à traiter, et sa réponse s'appelle une « solution de contournement » (workaround) — une réponse non planifiée à l'avance, car l'événement n'avait pas été anticipé. Côté analyse quantitative, quatre outils à distinguer par leur usage : la simulation de Monte-Carlo modélise l'effet combiné de multiples incertitudes sur le coût/délai global ; l'analyse de sensibilité isole quel risque a le plus fort impact individuel, restituée dans un diagramme en tornade (les risques les plus impactants en haut) ; l'arbre de décision structure un choix séquentiel sous incertitude, chaque branche pondérée par sa valeur monétaire attendue (EMV) ; le diagramme d'influence modélise des situations trop complexes pour un arbre de décision lisible. Enfin, l'AMDEC/FMEA (analyse des modes de défaillance et de leurs effets) est une technique structurée pour identifier CHAQUE mode de défaillance possible d'un composant, son effet et sa criticité — utile en amont, avant même la matrice probabilité-impact.",
      en: "A useful mnemonic: the five threat responses (avoid, transfer, mitigate, accept, escalate) each have an almost symmetric mirror on the opportunity side (avoid → exploit, transfer → share, mitigate → enhance, accept → accept, escalate → escalate) — memorizing the threat-side structure lets you recover the opportunity-side equivalent. The risk register (a detailed working document, continuously updated) must be distinguished from the risk report (a governance-facing summary presenting the project's overall risk exposure). Escalating means passing a risk to a decision level beyond the project manager's authority (program, portfolio, leadership) — it is neither a threat response nor an admission of failure, it is the correct answer when a risk falls outside the project's scope of action. Central exam point: RISK and ISSUE are NOT synonyms. A risk is FUTURE-oriented (an event that might happen), can be positive OR negative, is logged in the risk register, and its response is called a “risk response”. An issue is PRESENT-oriented (a situation that already exists), is ALWAYS negative, is logged in the issue log, and its response is called a “workaround” — an unplanned response, since the event was not anticipated in advance. On quantitative analysis, four tools to distinguish by use: Monte Carlo simulation models the combined effect of multiple uncertainties on overall cost/schedule; sensitivity analysis isolates which risk has the strongest individual impact, rendered as a tornado diagram (highest-impact risks on top); a decision tree structures a sequential choice under uncertainty, each branch weighted by its expected monetary value (EMV); an influence diagram models situations too complex for a readable decision tree. Finally, FMEA (Failure Mode and Effects Analysis) is a structured technique to identify EVERY possible failure mode of a component, its effect and its criticality — useful upstream, even before the probability-impact matrix.",
    },
    examples: {
      fr: "• Menace : atténuer en ajoutant un cycle de test supplémentaire pour réduire la probabilité d'une anomalie critique.\n• Opportunité : exploiter en accélérant le calendrier pour profiter d'un changement réglementaire favorable et lancer avant la concurrence.\n• Escalader un risque qui dépasse l'appétence au risque du projet vers le niveau programme, car il sort de l'autorité du chef de projet à l'accepter seul.\n• « Le fournisseur pourrait livrer en retard » → un risque (futur, incertain) ; « le fournisseur a livré en retard hier » → un point à traiter (présent, déjà survenu) qui appelle une solution de contournement immédiate.\n• Un diagramme en tornade révèle que le risque « disponibilité des briques » domine largement tous les autres facteurs de coût → concentrer l'attention et les réserves sur ce risque en priorité.",
      en: "Threat: mitigate by adding an extra QA cycle to reduce the probability of a critical defect.\nOpportunity: exploit by fast-tracking the schedule to benefit from a favorable regulatory change and launch ahead of competitors.\nEscalating a risk that exceeds the project's risk appetite to the program level, since it is beyond the project manager's authority to accept alone.\n“The vendor might deliver late” → a risk (future, uncertain); “the vendor delivered late yesterday” → an issue (present, already happened) calling for an immediate workaround.\nA tornado diagram reveals that the “brick availability” risk dwarfs every other cost factor → focus attention and reserves on that risk first.",
    },
  },
  {
    id: 'tp-schedule',
  ...
  },
  {
    id: 'tp-ethics',
    category: 'topic',
    title: { fr: "Code d'éthique et de déontologie professionnelle", en: 'Code of Ethics and Professional Conduct' },
    summary: {
      fr: '4 valeurs non négociables : responsabilité, respect, équité, honnêteté.',
      en: '4 non-negotiable values: responsibility, respect, fairness, honesty.',
    },
    body: {
      fr: "Le code du PMI repose sur 4 valeurs : responsabilité (assumer ses décisions et leurs conséquences), respect (considération des personnes et des ressources), équité (impartialité, sans favoritisme) et honnêteté (dire la vérité, transparence). Sur l'examen, une option qui viole l'une de ces valeurs est automatiquement FAUSSE, même si elle semble pragmatique ou protège le projet à court terme. Le PMI ne teste pas « ce qui est légal » mais ce qui est éthiquement supérieur : face à 4 réponses, éliminer d'abord celles qui mentent, cachent, contournent une règle ou favorisent quelqu'un ; garder celle qui protège les parties prenantes, respecte les règles, est transparente et agit avec intégrité.",
      en: "The PMI code rests on 4 values: responsibility (own your decisions and their consequences), respect (consideration for people and resources), fairness (impartiality, no favoritism) and honesty (tell the truth, be transparent). On the exam, an option that violates any one of these values is automatically WRONG, even if it looks pragmatic or protects the project short-term. The PMI does not test “what is legal” but what is ethically superior: facing 4 answers, first eliminate those that lie, hide, bypass a rule, or favor someone; keep the one that protects stakeholders, respects rules, is transparent, and acts with integrity.",
    },
    details: {
      fr: "Une technique d'élimination rapide fonctionne pour la plupart des questions d'éthique : rayer toute option qui ment, cache un fait, contourne un processus ou favorise une personne/un groupe ; ce qui reste protège les parties prenantes et respecte les règles — c'est presque toujours la bonne réponse. Sept réflexes reviennent systématiquement à l'examen : (1) toujours dire la vérité, même si cela fait mal ou retarde le projet ; (2) toujours divulguer un conflit d'intérêts dès qu'il apparaît, jamais après ; (3) ne jamais cacher une erreur — la corriger et en informer immédiatement, avec zéro tolérance pour la fraude ou la corruption ; (4) respecter les lois et règlements même quand une pratique locale les contredit ; (5) ne jamais discriminer, l'équité est totale ; (6) protéger les informations confidentielles, la confidentialité est une obligation, pas une option ; (7) signaler les comportements non éthiques observés, même si c'est inconfortable ou vise un collègue. Ces réflexes s'appliquent identiquement en contexte prédictif, agile ou hybride : l'éthique n'est jamais soumise au tailoring.",
      en: "A fast elimination technique works for most ethics questions: cross out any option that lies, hides a fact, bypasses a process, or favors a person/group; what remains protects stakeholders and respects rules — that is almost always the right answer. Seven reflexes recur systematically on the exam: (1) always tell the truth, even if it hurts or delays the project; (2) always disclose a conflict of interest as soon as it appears, never after; (3) never hide a mistake — correct it and report it immediately, with zero tolerance for fraud or corruption; (4) respect laws and regulations even when a local practice contradicts them; (5) never discriminate, fairness is total; (6) protect confidential information, confidentiality is an obligation, not an option; (7) report observed unethical behavior, even if uncomfortable or it targets a colleague. These reflexes apply identically in predictive, agile or hybrid contexts: ethics is never subject to tailoring.",
    },
    examples: {
      fr: "• Une erreur de calcul de budget est détectée après validation : la bonne réponse est de la corriger et d'en informer immédiatement les parties prenantes, pas de la dissimuler en espérant qu'elle passe inaperçue.\n• Un fournisseur potentiel se révèle être une entreprise dirigée par un proche du chef de projet : la bonne réponse est de divulguer ce conflit d'intérêts avant toute décision d'attribution, pas de rester discret « puisqu'on sait être impartial ».\n• Un supérieur hiérarchique demande de présenter un statut plus favorable que la réalité pour rassurer un comité : la bonne réponse est de résister et de présenter les faits exacts, même si cela déplaît.\n• Un(e) collègue commet une faute grave (falsification de données) : la bonne réponse est de la signaler par les canaux appropriés, pas de la couvrir par loyauté ni de se contenter d'une critique informelle.",
      en: "A budget calculation error is discovered after sign-off: the right answer is to correct it and inform stakeholders immediately, not to hide it hoping it goes unnoticed.\nA prospective vendor turns out to be run by someone close to the project manager: the right answer is to disclose this conflict of interest before any award decision, not to stay quiet “since you know you're impartial”.\nA superior asks you to present a status rosier than reality to reassure a committee: the right answer is to push back and present the accurate facts, even if it displeases them.\nA colleague commits serious misconduct (falsifying data): the right answer is to report it through the proper channels, not cover it out of loyalty or settle for informal criticism.",
    },
  },
]
